...
module mmu_feeder(
    ...
    //done signal for output
    assign done = en && (mmu_cycle >= 3'b010);
    // Assert clear only on first MMU cycle (pulse systolic accumulators once per op).
    assign clear = en && (mmu_cycle == 3'd0);
    ...
    assign b_data1 = en ?
                        (mmu_cycle == 3'd1) ? (transpose ? input2 : input1) :
                        (mmu_cycle == 3'd2) ? input3 : 8'd0
                        : 8'd0;
